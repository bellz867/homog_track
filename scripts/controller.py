...
def controller():
    ...
    velPub = rospy.Publisher('/bebop/cmd_vel',Twist,queue_size=1)
    velPub.publish(Twist()) # zero out previous commands
    launchPub = rospy.Publisher('/bebop/takeoff',Empty,queue_size=1)
    landPub = rospy.Publisher('/bebop/land',Empty,queue_size=1)
    resetPub = rospy.Publisher('/bebop/reset',Empty,queue_size=1)
...
def joyCallback(data):
    ...
    if override: 
        joyVel = Twist()
        (joyVel.linear.x,joyVel.linear.y,joyVel.linear.z) = (deadband(data.axes[3]),deadband(data.axes[2]),deadband(data.axes[1]))
        joyVel.angular.z = -1.0*deadband(data.axes[0])
