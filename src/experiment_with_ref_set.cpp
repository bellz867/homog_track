	...
	public:
		...
		void joy_callback(const sensor_msgs::Joy& msg)
		{
			...
			if (rb_button_teleop_b5 > 0)
			{
				...
				br.sendTransform(tf::StampedTransform(camera_wrt_body, time_temp, "ardrone", "camera_image"));
				tf::StampedTransform reference_wrt_world_temp;
				listener.waitForTransform("world", "camera_image", time_temp, ros::Duration(1.0));
				listener.lookupTransform("world", "camera_image", time_temp, reference_wrt_world_temp);
		...
		{
			//std::cout << "entered desired pixels callback" << std::endl;
			...
			//std::cout << "end of desired pixels callback" << std::endl;
