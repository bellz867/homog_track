...
class ImageConverter
{
	public:
		...
		ImageConverter() : it_(nh_)
		{
			...
			vir_R = cv::Mat::eye(3,3,CV_64F);
			vir_T = cv::Mat::zeros(3,1,CV_64F);
			///////////// rotation and translation from desired to the reference virtual camera /////////
			vir_T.at<double>(0,0) = -1;
			vir_T.at<double>(1,0) = -1;
			vir_T.at<double>(2,0) = -1;
...
{   
	...
	if (image_converter.using_virtual_reference)
	{
		...
		while (ros::ok())
		{
			...
			image_converter.homog_circles_virtual.header.stamp = ros::Time::now();
