	...
	public:
		...
		void image_callback(const sensor_msgs::ImageConstPtr& msg)
		{
			...
			try
			{
				...
				// if any of the pixels are negative then the features have not been found, otherwise they have
				if ((red_circle_p_curr.x < 0) || (green_circle_p_curr.x < 0) || (cyan_circle_p_curr.x < 0) || (purple_circle_p_curr.x < 0))
				{
...
class HomogDecomp
{
	public:
		...
		ros::Subscriber pixel_sub, ref_sub;// pixel and joy subscriber
		...
		void ref_callback(const homog_track::RefMsg& msg)
		{
			...
			ref_pixels.clear();
			ref_points_m.clear();
			pr_ref.setValue(msg.ref_cam_pixels.pr.x,msg.ref_cam_pixels.pr.y,msg.ref_cam_pixels.pr.z);
			pg_ref.setValue(msg.ref_cam_pixels.pg.x,msg.ref_cam_pixels.pg.y,msg.ref_cam_pixels.pg.z);
			pc_ref.setValue(msg.ref_cam_pixels.pc.x,msg.ref_cam_pixels.pc.y,msg.ref_cam_pixels.pc.z);
			pp_ref.setValue(msg.ref_cam_pixels.pp.x,msg.ref_cam_pixels.pp.y,msg.ref_cam_pixels.pp.z);
			
			ref_cam_pixels = msg.ref_cam_pixels;
		...
		void pixels_callback(const homog_track::ImageProcessingMsg& msg)
		{
			...
			if (msg.features_found.data && ref_msg.reference_set.data)
			{	
			...
			// if the decomp is greater than 0 it was successful
			if (successful_decomp > 0)
			{
			...
			{
				decomposed_msg.decomp_successful.data = false;
			}
...
class ReferenceSetter
{
	public:
		...
		ros::Publisher ref_pub;// publisher for the reference message
		...
		homog_track::RefMsg ref_msg_out;
		
		/********** joy declarations **********/
		int right_bumper = 0;//indicates if right bumper is pulled
		...
		void pixels_callback(const homog_track::ImageProcessingMsg& msg)
		{
			if (msg.features_found.data)
			{
				last_cam_pixels = msg;// update the last pixels recieved
			}
		}
		
		///********** callback for the controller from controller **********/
		void joy_callback(const sensor_msgs::Joy& msg)
		{
			right_bumper = msg.buttons[5]; // right bumper pulled is a 1 otherwise a 0
			if (right_bumper > 0)// right bumper pulled indicates the desire to set the reference
			{
				listener.waitForTransform("world", "bebop_image", ros::Time(0), ros::Duration(0.1));
				listener.lookupTransform("world", "bebop_image", ros::Time(0), reference_wrt_world);
				reference_wrt_world.child_frame_id_ = "reference_image";// update the child frame id
				if (std::abs(reference_wrt_world.stamp_.toSec() - last_cam_pixels.header.stamp.toSec()) < time_tolerance )// if the elapsed time between the pixels and the transform is not greater than the time tolerance will attempt to set reference
				{
					try
					{
						// getting the normal
						tf::Quaternion n_star_4 = ((reference_wrt_world.getRotation().inverse())*tf::Quaternion(n_world.getX(),n_world.getY(),n_world.getZ(),0))*reference_wrt_world.getRotation();
						n_star.setValue(n_star_4.getX(), n_star_4.getY(), n_star_4.getZ());
						...
						/********** message out **********/
						// reference pixels
						ref_msg_out.ref_cam_pixels = last_cam_pixels;
						
						// timestamp will be the same as the reference
						ref_msg_out.header.stamp = last_cam_pixels.header.stamp;
						
						// reference wrt world
						ref_msg_out.reference_wrt_world.translation.x = reference_wrt_world.getOrigin().getX(); ref_msg_out.reference_wrt_world.translation.y = reference_wrt_world.getOrigin().getY(); ref_msg_out.reference_wrt_world.translation.z = reference_wrt_world.getOrigin().getZ();
						ref_msg_out.reference_wrt_world.rotation.x = reference_wrt_world.getRotation().getX(); ref_msg_out.reference_wrt_world.rotation.y = reference_wrt_world.getRotation().getY(); ref_msg_out.reference_wrt_world.rotation.z = reference_wrt_world.getRotation().getZ(); ref_msg_out.reference_wrt_world.rotation.w = reference_wrt_world.getRotation().getW();
						
						// red wrt reference
						ref_msg_out.red_wrt_reference.translation.x = red_wrt_reference.getOrigin().getX(); ref_msg_out.red_wrt_reference.translation.y = red_wrt_reference.getOrigin().getY(); ref_msg_out.red_wrt_reference.translation.z = red_wrt_reference.getOrigin().getZ();
						ref_msg_out.red_wrt_reference.rotation.x = red_wrt_reference.getRotation().getX(); ref_msg_out.red_wrt_reference.rotation.y = red_wrt_reference.getRotation().getY(); ref_msg_out.red_wrt_reference.rotation.z = red_wrt_reference.getRotation().getZ(); ref_msg_out.red_wrt_reference.rotation.w = red_wrt_reference.getRotation().getW();
						
						// green wrt reference
						ref_msg_out.green_wrt_reference.translation.x = green_wrt_reference.getOrigin().getX(); ref_msg_out.green_wrt_reference.translation.y = green_wrt_reference.getOrigin().getY(); ref_msg_out.green_wrt_reference.translation.z = green_wrt_reference.getOrigin().getZ();
						ref_msg_out.green_wrt_reference.rotation.x = green_wrt_reference.getRotation().getX(); ref_msg_out.green_wrt_reference.rotation.y = green_wrt_reference.getRotation().getY(); ref_msg_out.green_wrt_reference.rotation.z = green_wrt_reference.getRotation().getZ(); ref_msg_out.green_wrt_reference.rotation.w = green_wrt_reference.getRotation().getW();
						
						// cyan wrt reference
						ref_msg_out.cyan_wrt_reference.translation.x = cyan_wrt_reference.getOrigin().getX(); ref_msg_out.cyan_wrt_reference.translation.y = cyan_wrt_reference.getOrigin().getY(); ref_msg_out.cyan_wrt_reference.translation.z = cyan_wrt_reference.getOrigin().getZ();
						ref_msg_out.cyan_wrt_reference.rotation.x = cyan_wrt_reference.getRotation().getX(); ref_msg_out.cyan_wrt_reference.rotation.y = cyan_wrt_reference.getRotation().getY(); ref_msg_out.cyan_wrt_reference.rotation.z = cyan_wrt_reference.getRotation().getZ(); ref_msg_out.cyan_wrt_reference.rotation.w = cyan_wrt_reference.getRotation().getW();
...
class Controller
{
	public:
		...
		void decomp_callback(const homog_track::DecompMsg& msg)
		{
			...
			if (msg.decomp_successful.data)
			{
				// because the reference is set to the exact value when when n should have only a z componenet, the correct
				// choice should be the one closest to n_star = [0,0,1]^T which will be the one with the greatest dot product with n_star
				...
				if (msg.n1[2] != -1 && msg.n2[2] != -1)
				{
					...
					if (n1_temp.dot(n_star) >= n2_temp.dot(n_star))
					{
						for (int ii = 0; ii < 9; ii++)
						{
